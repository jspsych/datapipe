...
app.use((req, res, next) => {
    ...
    const isAuthHeaderPresent = (`Bearer valid` === req.headers['authorization']);
  
    if (!areHeadersPresent || !isAuthHeaderPresent) {
      // If not, send an error response
      return res.status(400).json({ error: 'Missing required headers', headers: req.headers['authorization']});
    }

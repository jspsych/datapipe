...
beforeAll(async () => {
  ...
  await db.collection("experiments").doc('test-exp').set({active: true, metadataActive: true, owner: 'test-user', osfFilesLink: "http://localhost:3000/endpoint"});
  await db.collection('users').doc('test-user').set({osfTokenValid: true, osfToken: 'valid'});
  await db.collection("metadata").doc('test-exp').set({});
});
...
describe('runTransaction', () => {
  ...
  it('should handle the case when metadata is neither in firestore nor OSF', async () => {
    ...
    const docExp = await db.collection("experiments").doc("test-exp").get()
    const userExp = await db.collection("users").doc(docExp.data().owner).get()
    const metaExp = await db.collection("metadata").doc("test-exp").get()
    // console.log(docExp.data(), userExp.data(), metaExp.data())
  ...
  it('should handle the case when metadata is in OSF and in firestore', async () => {
    ...
    await db.collection("metadata").doc("test-exp").set({metadata: "test-metadata"}, {merge: true});

...
export default async function updateMetadata(existingMetadata, newMetadata) {
     ...
     */
    const filterVariables = (variables, excludeList, exclude) => variables.filter(variable => exclude === excludeList.includes(variable.name));
    /**
     ...
     */
    const findVariableIndex = (variableName, variableList) => variableList.findIndex(variable => variable.name === variableName);
    /**
    ...
    const updateLevels = (existingVariable, newVariable) => {
        ...
        newVariable.levels.forEach(newLevel => {
            var _a, _b;
            if (!((_a = existingVariable.levels) === null || _a === void 0 ? void 0 : _a.includes(newLevel))) {
                (_b = existingVariable.levels) === null || _b === void 0 ? void 0 : _b.push(newLevel);
            }
        });
    };
    /**
    ...
    for (const existingVariable of variablesToUpdate) {
        ...
        if (newVariableIndex === -1)
            continue; // Handle case when newVariable doesn't exist - for now it doesn't do anything.
    ...
    newVariablesNonDefault.forEach(((newVariable) => {
        if (findVariableIndex(newVariable.name, variablesToUpdate) === -1)
            variablesToUpdate.push(newVariable);
    }));
...
//# sourceMappingURL=metadata-update.js.map
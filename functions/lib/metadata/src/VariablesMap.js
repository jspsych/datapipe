...
export class VariablesMap {
    ...
    getList() {
        ...
        for (const key of Object.keys(this.variables)) {
            ...
            if (numKeys === 0)
                console.error("Empty description"); // error: description empty
            else if (numKeys === 1) {
    ...
    setVariable(variable) {
        ...
        const unexpectedFields = Object.keys(variable).filter((key) => ![
            "type",
            "name",
            "description",
            "value",
        ...
        if (unexpectedFields.length > 0) {
            console.warn(`Unexpected fields (${unexpectedFields.join(", ")}) detected and included in the variable object.`);
        }
    }
    /**

...
export default async function blockMetadata(exp_data, user_data, metadata_doc_ref, data, metadataOptions) {
    let metadataMessage = { metadataMessage: '' };
    try {
        ...
        if (exp_data.metadataActive) {
            ...
            await db.runTransaction(async (t) => {
                ...
                if ((osfMetadataId && firestoreMetadata) || (!osfMetadataId && firestoreMetadata)) {
                    ...
                    if (osfMetadataId) {
                        await updateFileOSF(exp_data.osfFilesLink, user_data.osfToken, JSON.stringify(updatedMetadata, null, 2), osfMetadataId);
                    }
                    //If a metadata file does not exist in OSF, it is created with the above metadata.
                    else {
                        await putFileOSF(exp_data.osfFilesLink, user_data.osfToken, JSON.stringify(updatedMetadata, null, 2), `dataset_description.json`);
                    }
                }
                ...
                if (osfMetadataId && !firestoreMetadata) {
                    ...
                    const osfMetadata = downloadResponse.metadata;
                    ...
                    await updateFileOSF(exp_data.osfFilesLink, user_data.osfToken, JSON.stringify(incomingMetadata, null, 2), osfMetadataId);
                }
                ...
                if (!osfMetadataId && !firestoreMetadata) {
                    ...
                    await putFileOSF(exp_data.osfFilesLink, user_data.osfToken, JSON.stringify(incomingMetadata, null, 2), `dataset_description.json`);
                }
            });
            const metadataResponse = { success: true, ...metadataMessage };
        ...
        const metadataResponse = { success: false, ...MESSAGES.METADATA_ERROR, message: errorMessage, ...metadataMessage };
        return metadataResponse;
        //METADATA BLOCK END
    }
    ;

  ...
  return (
    <>
      <PageHeader
        ...
        purpose="How DataPipe checks incoming data before storing it, and how to cap the number of sessions an experiment accepts."
      />
      ...
      <DocsSection id="how-validation-works" title="How validation works">
        <Text maxW="70ch">
          ...
          storage provider and rejects anything that doesn&apos;t match the
          rules you set. You can require a file to be well-formed JSON or CSV,
          and you can list columns or fields it must contain.
        </Text>
        <Text maxW="70ch">
          <strong>Validation is on for every new experiment</strong>, with
          JSON and CSV both allowed and one required field:{" "}
          <Code>trial_type</Code>. This is the most common reason a first test
          submission gets rejected. Data that didn&apos;t come from jsPsych
          has no <Code>trial_type</Code> column, so it fails the check until
          you edit or clear the required fields on the dashboard.
        </Text>
        <Text maxW="70ch">
          A submission passes if it&apos;s well-formed in <em>either</em> of
          the formats you allow. A rejection comes back as a <Code>400</Code>{" "}
          with <Code>INVALID_DATA</Code>: &ldquo;The data are not valid
          according to the validation parameters set for this
          experiment.&rdquo; DataPipe doesn&apos;t say which check failed, so
          run a real session yourself before you recruit.
        </Text>
      </DocsSection>

      <DocsSection id="required-fields" title="Required fields">
        <Text maxW="70ch">
          Required fields are matched against the top level of your data only.
          A field nested inside another object doesn&apos;t count, however
          deep it is.
        </Text>
        <List.Root maxW="70ch" gap={3} ps={6}>
          <List.Item>
            ...
            the usual shape for jsPsych output. DataPipe collects the keys from
            every object in the array and requires each of your fields to
            appear in at least one of them. A field that only one trial
            carries still counts as present.
          </List.Item>
          <List.Item>
            <Text as="span" fontWeight="semibold">
          ...
          <List.Item>
            ...
            one extra rule: <strong>every row must have the same number of
            cells as the first row</strong>. A ragged file is rejected even if
            you haven&apos;t listed any required fields. The usual culprit is
            an unquoted comma inside a response.
          </List.Item>
        </List.Root>
        <Text maxW="70ch">
          On the dashboard, type a name and press Enter or comma to add it, or
          paste a comma-separated list all at once. Each name becomes a chip
      ...
      <DocsSection id="rejected-data-is-gone" title="Rejected data is gone">
        <Box borderWidth="1px" borderColor="border" bg="bg.muted" rounded="md" p={4} maxW="70ch">
          <Text fontSize="sm">
            ...
            never reaches your storage provider, never enters the upload
            queue, and never appears on your dashboard. There is no copy to
        ...
        <Text maxW="70ch">
          ...
          which is why nothing survives it. Validation is there to block bad
          submissions, not to catch mistakes in legitimate data.
        </Text>
        <Text maxW="70ch">
          So set your rules deliberately, and test them once, end to end, with
          your real experiment before the first participant arrives.
        </Text>
      </DocsSection>
      ...
      <DocsSection id="session-limits" title="Session limits">
        <Text maxW="70ch">
          A session limit keeps an experiment from overshooting its
          recruitment target. Once the number of accepted submissions reaches
          the limit, every further submission is rejected with{" "}
          <Code>SESSION_LIMIT_REACHED</Code>. You can raise the limit at any
          time and collection resumes immediately. The field starts at 1, so
          set it when you turn the switch on.
        </Text>
        <Text maxW="70ch">
        ...
        <List.Root maxW="70ch" gap={3} ps={6}>
          <List.Item>
            ...
            If DataPipe accepted the data but hasn&apos;t delivered it to your
          ...
          <List.Item>
            ...
            are neither counted nor blocked by the cap, so an experiment at
            its limit can still receive media files.
          </List.Item>
          <List.Item>
            <Text as="span" fontWeight="semibold">
              A burst can overshoot slightly.
            </Text>{" "}
            The count is read when a submission arrives and written when
            it&apos;s accepted. Several participants who submit in the same
            instant can each pass the check before any of them is counted, so
            a cap of 50 can end up with a few more than 50 sessions.
          </List.Item>
          <List.Item>
            <Text as="span" fontWeight="semibold">
        ...
        <Text maxW="70ch">
          Because of the last two points, treat the limit as a guard rail
          rather than an exact quota. If you need exactly n complete datasets,
          check the count on your dashboard before you close recruitment.
        </Text>
      </DocsSection>
      ...
      <DocsSection id="security-posture" title="Security posture">
        ...
        <Text maxW="70ch">
          Anyone who reads your experiment&apos;s code can see its experiment
          ID, and that ID is all it takes to submit. That&apos;s the trade
          DataPipe makes so participants don&apos;t need accounts and you
          don&apos;t need a server. Validation, the session limit, and turning
          off data collection when a study ends are the three controls that
          keep it in check.
        </Text>
        <GuidanceLine href="/docs/data" linkText="What DataPipe stores">
          What reaches DataPipe, how long it&apos;s kept, and who can read it.
        </GuidanceLine>
      </DocsSection>
    </>
  );

...
// A reset link that has already been used or has aged out. The page that
// requested the reset needs to fall back to its "enter your email again"
// state rather than dead-end on this code -- see reset-password.js.
const EXPIRED_RESET_LINK = new Set([
  "auth/expired-action-code",
  "auth/invalid-action-code",
]);

export function isExpiredResetLink(code) {
  return EXPIRED_RESET_LINK.has(code);
}
...
const FALLBACK = {
  ...
  password: () => "Could not sign you in. Please try again.",
  ...
  resetRequest: () => "Could not send the reset email. Please try again.",
  resetConfirm: () => "Could not reset your password. Please try again.",
};
...
export function messageForAuthError(
  ...
  switch (code) {
    ...
    case "auth/user-not-found":
      // Modern Firebase (email enumeration protection on) collapses "wrong
      // password" and "no such account" into the single code
      // auth/invalid-credential, so the two are indistinguishable from
      // here; a project with protection off, or an older SDK, can still
      // surface auth/wrong-password or auth/user-not-found directly. All
      // three get the same neutral copy in "password" mode -- guessing
      // which field is at fault would be exactly the account-existence
      // confirmation email enumeration protection exists to prevent, and
      // for auth/invalid-credential there is no way to guess right anyway.
      if (mode === "password") {
        return "We couldn't sign you in with that email and password. Double-check both, or reset your password if you've forgotten it.";
      }
    ...
    case "auth/user-disabled":
      return "This account has been disabled. Get in touch through the Contact page.";

...
/**
 * The StatusIndicator status for a queue tone. One mapping, used by the panel's
 * headline and the page-header chip, so the two can never disagree: a queue in
 * which everything is merely waiting is `waiting` (a clock), not `neutral` (a
 * minus sign, which beside "N files are waiting to be stored" reads as a dash).
 * The tone vocabulary itself stays error | warning | neutral -- it also picks
 * the panel's chrome, where "neutral" means exactly that.
 */
export function queueToneStatus(tone) {
  return tone === "neutral" ? "waiting" : tone;
}

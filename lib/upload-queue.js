 ...
 *
 * WHAT "NEVER ATTEMPTED" MEANS IN THE REAL FIELD SHAPES
 *
 ...
 * simply absent, so `== null` covers the one shape actually written. The
 * retry worker (scheduled-upload-retry.ts's processQueueItem) is the ONLY
 * thing that ever sets `lastAttemptAt`, and it does so atomically with
 * flipping `status` to "processing" the moment it claims a doc -- before it
 * has attempted a provider write. So `lastAttemptAt != null` means "the retry
 * worker has looked at this at least once", which is the right line: an entry
 * the compaction gate bounces straight back to "pending" without touching the
 * provider (scheduled-upload-retry.ts's `isCompactionInFlight` branch) keeps
 * its `failureReason` of "Compaction in progress" but now carries a real
 * `lastAttemptAt`, and correctly reads as "retrying" from here on -- it has
 * been through the worker once, even though that pass didn't write anywhere.
 *
...
export function queueEntryKind(entry) {
  ...
  const neverAttempted =
    entry?.lastAttemptAt == null && (entry?.retryCount ?? 0) === 0;

  if (neverAttempted && isHeldReason(entry?.failureReason)) return "waiting";

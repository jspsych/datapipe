export async function validateOsfAccess(osfToken, componentId) {
  try {
    const response = await fetch(`https://api.osf.io/v2/nodes/${componentId}/`, {
      headers: {
    ...
    if (!response.ok) {
      if (response.status === 403) {
        ...
        throw new Error('Failed to validate OSF project access.');
      }
    }
...
export async function getOsfComponentInfo(osfToken, componentId) {
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch OSF component information');
    }

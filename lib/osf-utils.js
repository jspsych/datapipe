...
export async function getOsfComponentInfo(osfToken, componentId) {
  try {
    ...
    return {
      ...
      region: data.data.relationships?.region?.data?.id || 'us'
    };
  } catch (error) {
    throw error;
  }
...
export function cleanOsfUrl(osfUrl) {
  ...
  // Remove OSF URL prefix if present (handles both production and test environments)
  osfUrl = osfUrl.replace(/^https?:\/\/(?:test\.)?osf\.io\//, "");

  // Remove trailing slash if present
  if (osfUrl.endsWith("/")) {
    osfUrl = osfUrl.slice(0, -1);
  }

  return osfUrl;
}

export function generateOsfComponentName(baseName) {

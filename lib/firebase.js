...
try {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      // Always use emulator config in development
      firebaseConfig = {
      ...
      console.log("Created new Firebase app with emulator config");
    } else if (process.env.NEXT_PUBLIC_FIREBASE_CONFIG) {
      // Use production config only in production
      firebaseConfig = JSON.parse(process.env.NEXT_PUBLIC_FIREBASE_CONFIG);
      app = initializeApp(firebaseConfig);
      console.log("Created new Firebase app with production config");
    } else {
      // Fallback to emulator config
      firebaseConfig = {
        projectId: "demo-project",
        apiKey: "demo-api-key",
        authDomain: "demo-project.firebaseapp.com",
      ...
      app = initializeApp(firebaseConfig);
      console.log("Created new Firebase app with fallback emulator config");
    }
  }
} catch (error) {

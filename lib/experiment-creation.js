  ...
  if (userdoc.exists()) {
    ...
    if (data.usingPersonalToken) {
      ...
      if (data.authTokenExpires > Date.now()) {
        ...
        // Token refresh
        const idToken = await user.id;
        const regenResult = await fetch('/api/oauth2regenerate', {
          method: "POST",
          headers: {
        ...
        if (!regenResult.ok) {
          const errorText = await regenResult.text();
          
          // Handle specific OAuth errors
          if (regenResult.status === 400) {
            try {
              const errorData = JSON.parse(errorText);
              if (errorData.details && errorData.details.includes('invalid_request')) {
                throw new Error('Your OSF authentication has expired. Please sign out of DataPipe and sign in again with OSF.');
              }
            } catch (parseError) {
          ...
          throw new Error(`Token refresh failed: ${regenResult.status}`);
        }
